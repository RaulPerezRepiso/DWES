...
class ACLBD extends ACLBase
{
    ...
    function esValido(string $nick, string $contrasena): bool
    {
        ...
        $sentencia = "SELECT contrasenia FROM acl_usuarios WHERE nick='{$nick}' AND borrado = 0";
        $resul = $this->_sqli->query($sentencia);
        ...
        return password_verify($contrasena, $fila["contrasenia"]);
    }
    ...
    {
        //Si hay conexíon empezmaos a vaidar cod usuario y que permisos tienen
        if (!$this->_hayConeccion)
            return false;

        // Obtener el rol del usuario 
        $codRol = $this->getUsuarioRole($codUsuario);
        if ($codRol === false)
            return false;

        // Obtener los permisos asignados a ese rol que tiene 
        return $this->getPermisosRole($codRol);
    }
    ...
    {
        $sql = "SELECT cod_acl_role FROM acl_usuarios WHERE cod_acl_usuario = $codUsuario";
        $res = $this->_sqli->query($sql)->fetch_assoc();

        return $res ? intval($res["cod_acl_role"]) : false;
    }
    ...
    function setUsuarioRole(int $codUsuario, int $role): bool
    {
        ...
        $consulta = $this->_sqli->query($sentencia);
