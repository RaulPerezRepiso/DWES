...
/**
 * Esta función comprueba que $var contiene un entero
 * cuyo valor está entre $min y $max 
 * CON FILTER
 *
 * @param integer $var
 * @param integer $min
...
function validaReal(float &$var, float $min, float $max, float $defecto): bool
{
    ...
    if (filter_var($var, FILTER_VALIDATE_FLOAT, $opciones)) {
        return true;
    } else {
        $var = $defecto;
        return false;
 ...
 * contiene un email correcto en el formato aaaaa@bbbb.ccc. 
 * CON FILTER
 * 
 * @param string $var
 * @param string $defecto
...
function validaExpresion(string &$var, string $expresion, string $defecto): bool
{
    ...
    if (filter_var($var, FILTER_VALIDATE_REGEXP, $expresion)) {
        return true;
    } else {
        $var = $defecto;
        return false;
